\chapter{Skalenübergreifende Charakterisierung zweiphasiger Elektrolyte}\label{sec:sim_sbe}

Die numerische Beschreibung zweiphasiger Elektrolytsysteme erfordert aufgrund ihrer komplexen Porenmorphologie eine effiziente Reduktion der Mikrostrukturkomplexität unter Wahrung der physikalischen Genauigkeit (TZ 2). Um den Rechenaufwand konventioneller Feldsimulationen zu vermeiden, untersucht die hier vorliegende Methodik alternative Ansätze zur Ermittlung der effektiven Eigenschaften der Strukturelektrolyte. Zunächst ermöglicht eine Porennetzwerk-Modellierung die hocheffiziente Bestimmung der ionischen Leitfähigkeit $\gamma$ und des effektiven Diffusionskoeffizienten $D$ direkt aus experimentellen Absorptionsmessungen. Während dieser Ansatz eine schnelle Charakterisierung der Transporteigenschaften erlaubt, bleibt die mechanische Steifigkeit $\boldsymbol{C}$ unberücksichtigt. 

Um diese Lücke zu schließen, wird die \textit{Walk-on-Spheres}-Methode eingesetzt, die sowohl Diffusionskoeffizienten als auch effektive Steifigkeiten in komplexen, nicht"=templatebasierten\footnote{Während bei der Phasenseparation die poröse Architektur durch spontane Entmischung (Selbstorganisation) der Komponenten entsteht und meist zufällige Strukturen bildet, wird bei template-basierten Verfahren eine präzise 3D-Geometrie aktiv vorgegeben (z. B. durch Elektrospinning oder chemische Selbstorganisation). Dies ermöglicht eine gezielte Optimierung der Ionenleitfähigkeit durch geordnete Transportwege bei gleichzeitiger Maximierung der mechanischen Steifigkeit~\cite{Menner2006a, Shirshova2013a, Stubenrauch2018}.} Porenstrukturen simultan berechnen kann. Da dieses Verfahren jedoch auf eine präzise räumliche Geometrie angewiesen ist, wird die Methodik durch ein \textit{Convolutional Neural Network} (CNN) vervollständigt. Dieses sagt die Parameter der \textsc{Cahn-Hilliard}-Gleichung direkt aus REM-Aufnahmen voraus und automatisiert so die Generierung realistischer repräsentativer Volumenelemente (RVE). In der Summe entsteht eine geschlossene Simulationskette, die von der Bilddiagnostik bis zur Vorhersage makroskopischer Materialparameter eine recheneffiziente Skalierung gewährleistet.


\section{Modellierung der effektiven Ionenleitfähigkeit als Porennetzwerk}

Die Porennetzwerk-Modellierung (PNM) ist ein ursprünglich aus den Geowissenschaften stammendes Ersatzmodell, welches in dieser Arbeit für die Anwendung auf zweiphasige Elektrolyte adaptiert wurde \cite{Xiong2016, Gostick2016}. Hierbei wird die Prämisse zugrunde gelegt, dass der Ionentransport ausschließlich in der flüssigen Phase stattfindet, während das feste Polymernetzwerk als isolierend betrachtet wird \cite{Tu2020, zschiebsch2024pore}.
...
Die Grundlage bildet die Beschreibung des diffusiven Transports. Das erste \textsc{Fick}sche Gesetz beschreibt den molaren Ionenfluss $\boldsymbol{j}$ in Abhängigkeit des Konzentrationsgradienten:
...
\end{equation}
Aus der Massenbilanz für jede Pore $i$ und deren benachbarte Poren $j \in \text{Nb}_i$\footnote{$\text{Nb}$ ist die Anzahl an Nachbarporen der Pore $i$.} folgt im stationären Zustand:
\begin{equation}
...
\end{equation}
Unter Annahme kugelförmiger Poren mit Durchmesser $d$ und zylindrischer Verbindungen mit Durchmesser $d_{ij}$ und Länge $l$ (siehe Bild~\ref{fig:sphere_cylinder_model}) ergeben sich die Formfaktoren zu:
\begin{align}
...
Die Verknüpfung zur ionischen Leitfähigkeit $\gamma$ erfolgt über die \textsc{Nernst-Einstein}-Gleichung:
\begin{equation}
    D = \frac{k_{\text{B}}T}{n q^2} \gamma.
\end{equation}
Hieraus lässt sich eine modifizierte molare Flussrate $\dot{m}'$ ableiten, welche die ionische Leitfähigkeit direkt berücksichtigt:
\begin{equation}
...
\begin{figure}[!ht]
    ...
    \caption{\label{fig:sphere_cylinder_model_convergence}Analyse des relativen Fehlers: (a) Einfluss des Volumenverhältnisses, (b) Variation der Konnektivität, (c-e) Konvergenzstudie über die Porenanzahl.}
\end{figure}

Die Ergebnisse (Bild~\ref{fig:sphere_cylinder_model_convergence}) verdeutlichen, dass der Fehler mit zunehmender Netzwerkgröße sinkt. Ab einer Anzahl von ca. 3\,000 Poren stabilisiert sich der Fehler unterhalb von \SI{10}{\percent}. Es zeigte sich zudem, dass die Konnektivität (Anzahl der Verbindungen pro Pore) die effektive Leitfähigkeit maßgeblich beeinflusst (Bild~\ref{fig:sphere_cylinder_model_convergence}b). 
Die hier beschriebene Methode ermöglicht es, mit vergleichsweise geringem Rechenaufwand sehr genaue Voranalysen für den erwartbaren effizienten Diffusionskoeffizienten zu erhalten. Die dazu benötigten Parameter können dabei, wie gezeigt, aus weitverbreiteten Gasabsorptionsmessungen ermittelt werden. Solange nur die effektive Leitfähigkeit oder der Diffusionskoeffizient von Interesse sind, stellt die hier vorgestellte Porennetzwerk-Modellierung eine effiziente Alternative zu aufwendigeren Methoden dar. Für die Bestimmung der effektiven Steifigkeit ist jedoch ein anderer Ansatz notwendig, welcher im folgenden Abschnitt beschrieben wird.

\section{Effektive Steifigkeit und Diffusionskoeffizient mittels Walk-on-Stars}
...
\begin{figure}[!ht]
    \center
    \includegraphics[width=0.99\textwidth, angle=0]{method_wost.pdf}
...
Im Ergebnis liegt damit eine Methode vor, die es ermöglicht, sowohl die effektive Steifigkeit als auch den Diffusionskoeffizienten für komplexe Porenstrukturen zu bestimmen. Damit stellt WoSt eine wertvolle Ergänzung zur klassischen FE"=Methode dar. Ein wesentlicher Vorteil liegt in der Fähigkeit, hochauflösende Geometrien mit geringem Rechenaufwand zu behandeln, was insbesondere für die Simulation von Strukturelektrolyten von Bedeutung ist. Die Konvergenzanalyse (Bild~\ref{fig:wost_result}c-d) zeigt, dass bereits ab einer Stichprobengröße von ca. 100 Schritten eine Fehlerreduktion auf unter \SI{5}{\percent} erreicht werden kann. Allerdings wird wie auch bei der FE-Methode die Genauigkeit durch die Qualität der zugrunde liegenden Geometrie begrenzt, weshalb eine präzise RVE-Generierung von entscheidender Bedeutung ist. Die folgende Methodik zur CNN-basierten RVE-Generierung aus REM-Aufnahmen adressiert genau diese Herausforderung und ermöglicht die automatisierte Generierung von repräsentativen Volumenelementen für die WoSt-Simulationen.
...
Die Ergebnisse (Tabelle~\ref{tab:nn_method_rve_se_results}) zeigen, dass das Netzwerk die Parameter konsistent ableitet. Hohe $\mathcal{L}$-Werte zwischen 0,88 und 0,98 belegen eine exzellente Übereinstimmung mit den Experimenten. Die Parameter $M$\footnote{Die Mobilität ist im Kontext der Cahn-Hilliard-Gleichung ein Maß für die Geschwindigkeit der Phasenseparation.} und $T_{\text{end}}$\footnote{Zeitpunkt, bei dem die fortlaufende Phasenseparation gestoppt wird.} korrelieren hierbei stark mit der Koarsening-Dynamik: Eine hohe Mobilität führt zu einer schnelleren Ausbildung großer Porenstrukturen, wie beim System 50MTM57/2.3 beobachtet. 

\input{Abbildungen/03_Modellierung/generated_rve_se/generated_rve_se.tex}

Der Startwert $\varphi_0$ definiert das initiale Phasenverhältnis, wobei niedrigere Werte (z.B. polyMIPE) einen höheren Festphasenanteil repräsentieren~\cite{Wu2010,Jiang2016,Wu2022}. Der Koeffizient $\lambda$ bestimmt die Schärfe der Phasengrenzen; größere Werte führen zu klarer ausgeprägten Strukturen, was qualitativ mit den REM-Aufnahmen übereinstimmt. Damit stellt die ML-basierte Parametrisierung eine robuste Methode dar, um experimentell schwer zugängliche Gefügeparameter direkt aus der Bildgebung in physikalisch konsistente RVE-Modelle zu überführen.

Die hier beschriebene Methodik ermöglicht es, die komplexe Mikrostruktur zweiphasiger Strukturelektrolyte effizient zu generieren. Durch die Kombination von Porennetzwerk-Modellierung, Walk-on-Stars-Methode und CNN-basierter RVE-Generierung entsteht eine leistungsfähige Simulationskette, die zur Vorhersage makroskopischer Materialparameter dient und eine recheneffiziente Skalierung gewährleistet.

\section{Integration in die skalenübergreifende Simulationskette}

Die vorgestellte Methodik bildet die technologische Basis für die in Kapitel~\ref{sec:multi_scale} beschriebene gesamtheitliche Strukturbatteriesimulation. Während Standard-Mikrostrukturen (z.~B. Faser-Matrix-Verbunde) aufgrund ihrer geordneten Geometrie effizient mittels klassischer FEM homogenisiert werden können, adressiert der hier entwickelte WoS-Ansatz gezielt die stochastische Komplexität der Strukturelektrolyte.

Für das im Validierungsszenario (Kapitel~\ref{sec:validation}) verwendete Materialsystem aus LP30 und \SI{10}{\percent} Kynar Flex 28 wurden die erforderlichen \textsc{Cahn-Hilliard}-Parameter automatisiert über das trainierte CNN aus REM-Aufnahmen extrahiert. Aus den so generierten RVEs wurden mittels WoS die effektiven mechanischen Steifigkeiten sowie die Diffusionskoeffizienten bestimmt\footnote{Die automatisierten RVE-Generierung und Kennwertermittlung wird in Kapitel~\ref{sec:automated_rves} vertiefend behandelt.}, welche als Eingangswerte in die makroskopischen Look-up-Tabellen der Gesamtsimulation (Kapitel~\ref{sec:look_up_tables}) einfließen. Damit ist die physikalisch konsistente Abbildung des Elektrolyten sichergestellt, ohne den Rechenaufwand der globalen Bauteilauslegung durch eine direkte Auflösung der Nanoporenstruktur zu erhöhen.