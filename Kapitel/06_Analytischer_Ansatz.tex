\chapter{Analytische Vorauslegung von Strukturbatterien}\label{sec:analytical_approach}
...
Dadurch sind die einzelnen Schichten nicht direkt miteinander verbunden und halten einzig durch den Druck, der durch die äußere Pouchfolie ampliziert wird, aneinander. Unter der Annahme der Verschiebungsstetigkeit an den Schichtgrenzflächen folgt, dass die Krümmung $\kappa$ mit
\begin{equation}
    \label{eq:curvature}
...
\begin{figure}[!ht]
	%\raggedleft
		...
		\caption{\label{fig:bending_pre_tests}Vereinfachte Bestimmung des Verformungsverhaltens an \textbf{a)} 11-lagigen Aluminium-Probekörpern, gefügt mit \textbf{b)} variierenden Anteilen an PVDF und Luft. Vergleich der \textbf{c-d)} 3-Punkt-Biegeversuche mit den \textbf{e)} berechneten Kraft-Verschiebungswerten.}
\end{figure}
...
Die Gegenüberstellung der Ergebnisse aus Bild~\ref{fig:bending_pre_tests}e zeigt, dass der analytische Ansatz die experimentell beobachteten Verformungsverhalten innerhalb des ersten Millimeters relativ gut darstellt. Allerdings weicht mit zunehmender Verformung die lineare Annäherung vom beobachteten nicht-linearen Verhalten ab. Eine Hauptursache könnte hierbei der wachsende Einfluss der geometrischen Nichtlinearitäten sein. Dies würde auch erklären, warum die Abweichung vom linearen Verlauf bereits so frühzeitig zu beobachten ist, wo weitere Einflussfaktoren, wie Plastifizierung, noch keine signifikante Rolle spielen. Da dieser analytische Mischungsansatz aber bereits jetzt ermöglicht, die mechanische Wirksamkeit des infiltrierten Strukturelektrolyten ohne numerische Kontaktmodellierung direkt in die Dimensionierung der Hochleistungsstruktur einzubeziehen, ist dieser rein linearere Ansatz für eine erste analytische Beschreibung des Verhaltens als ausreichend zu bewerten.
...
Ein ebenfalls vielversprechender Ansatz lässt sich in Gleichung~\ref{eq:capacity_stack} finden. Die lineare Skalierung der Kapazität mit der Anzahl der Zellen legt nahe, dass die Stapelung von mehr Zellen eine direkte Möglichkeit darstellt, die Energiedichte zu erhöhen. Auch wirkt sich die damit verbundene größere Dicke sehr positiv auf die Biegesteifigkeit aus. In der Praxis führen jedoch die damit verbundenen Herstellungsherausforderungen, wie die Gewährleistung einer homogenen Zellverschaltung und die Vermeidung von Kurzschlüssen, zu einem Kompromiss, be dem häufig kleinere Zellstapel bevorzugt werden\footnote{Fast alle derzeit bekannten Strukturbatterien bestehen aus lediglich einer Zelle, konventionlle Batterien weisen bis etwa 20 Zellen pro Stack auf.}.
