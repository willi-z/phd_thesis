...
Hierbei beschreibt $\bar{\psi}_{\alpha}$ die intrinsische Mittelung über die Phase $\alpha$. Im Gegensatz zur klassischen Mittelung $\langle \psi_{\alpha} \rangle$, die sich auf das gesamte Volumen bezieht, bietet die intrinsische Mittelung eine höhere Flexibilität bei variierenden Phasenanteilen $\varepsilon_{\alpha}$, da die Kennwerte unabhängig vom Volumenanteil definiert werden können. Die Umrechnung erfolgt über den Zusammenhang $\langle \psi_{\alpha} \rangle = \varepsilon_{\alpha} \bar{\psi}_{\alpha}$~\cite{Doyle1995}.
...
\begin{enumerate}
    ...
    \begin{equation}
        \hat{j} = j(c_{\text{s,e}},\hat{c}_{\text{e}},\hat{\phi}_{\text{s}},\hat{\phi}_{\text{e}})
    \end{equation}
\end{enumerate}
...
\subsection{Geometrische Vereinfachung und P2D-Ansatz}
...
\begin{enumerate}
    \item Kugelförmige Partikel (z. B. NMC/LFP): 
    \begin{equation}
    ...
    \end{equation}
    \item Zylindrische Fasern (z. B. Kohlenstofffasern): 
    \begin{equation}
...
Die daraus folgenden Modelle gelten als die effizientesten physikalisch basierten Batteriemodelle~\cite{Plett2024}. In der vorliegenden Implementierung konnten damit mehrere Zyklen über einen realen Zeitraum von 65\,h in unter 43\,min simuliert werden\footnote{Unter voller Ausnutzung von zwei CPUs der Marke AMD EPYC 75F3 mit jeweils 32 Kernen.}. Dies entspricht einer Beschleunigung gegenüber dem direkten Homogenisierungsansatz nach \textsc{Carlstedt} um mehrere Größenordnungen. Trotz dieser massiven Vereinfachung bleibt die Genauigkeit hoch und zeigt in der Literatur meist Abweichungen von unter 0,5\,\% gegenüber höherdimensionalen Modellen~\cite{Pistorio2023}. Damit stellt der P2D-Ansatz das leistungsfähigste Werkzeug für die hier angestrebte skalenübergreifende Simulation dar, sofern die entsprechenden Materialparameter präzise identifiziert werden können.